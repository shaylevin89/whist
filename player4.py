...
class GameBoard:
    def __init__(self):
        ...
        self.player2 = Label(self.root, text='player 2', padx=200, pady=30, font=('arial', 18)).pack()
        self.name_label = Label(self.root, text='player 4', padx=50, pady=30, font=('arial', 18), bg='green')
        self.name_label.place(relx=0.4, rely=0.905)
        self.canvas = Canvas(width=80, height=340)
        self.canvas.create_text(40, 180, font=('arial', 18), text="player 1", angle=90)
        self.canvas.place(relx=0, rely=0.2)
        self.canvas2 = Canvas(width=80, height=340)
        self.canvas2.create_text(40, 180, font=('arial', 18), text="player 3", angle=270)
    ...
    def first_connect(self):
        while True:
            try:
                data = self.sock.recv(265).decode()
                if len(data) > 10:
                    self.cards = data.split(' ')[39:]
                    print(self.cards)
                    # self.make_butts(self.cards)
                elif data[-1] == '3':
